...
class IndexView {
    ...
    constructor() {
        ...
        let selectedOption = document.getElementById("email-contact-reasoning");

        for (let i = 0; i < dropdownOptions.length; i++) {
            let dropdownOption = dropdownOptions[i] as HTMLButtonElement;
            dropdownOption.addEventListener("click", () => {
                selectedOption.innerText = dropdownOption.value;
            });
        }
        ...
        this.emailForm.addEventListener('submit', (e) => {
            ...
            if (selectedOption.innerText == "") {
                alert("Select a reason for contact");
                return;
            }
            ...
            fetch("http://localhost:8080/event-email", {
                ...
                body: JSON.stringify({
                    ...
                    contactReason: selectedOption.innerText,
    ...
    showActiveEvent(slideShowNumber: number) {
        ...
        let image = document.getElementById('index-main-content-events-listing-bg-image');
        ...
        image.style.backgroundImage = `url('${this.viewEventsArr.at(slideShowNumber).imgSrc}')`;
    }

    showActiveEvent2() {
        let carousel = document.getElementById("item1");

        this.viewEventsArr.forEach((event) => {
            carousel.insertAdjacentHTML("afterend", `
        <div class="carousel-item">
            <img class="image-carousel-bootstrap" src="${event.imgSrc}" class="d-block w-100" alt="...">
